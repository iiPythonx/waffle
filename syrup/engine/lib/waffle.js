...
export class Waffle {
    constructor(emit) {
        ...
        // Live status
        this.live = false;  // Assume manual stepping by default
    ...
    step() {
        ...
        if (!this.live) this.emit("waffle_instruction_log", { offset, instruction, args });
        ...
        // Send registers
        if (!this.live) this.emit("waffle_register_update", {
            1: this.read_reg(0x0),
            2: this.read_reg(0x1),
            3: this.read_reg(0x2),
            4: this.read_reg(0x3),
            ...
            8: this.read_reg(0x7),
            9: this.read_reg(0x8),
            lc: this.read_reg(0xA),
            cr: this.read_reg(0xB),
            sp: this.read_reg(0xC),
        });
    }
}
